...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("sentiment_analyzer.log"), logging.StreamHandler()],
)
...
def validate_text_input(text_input: str) -> Dict[str, Any]:
    ...
    if not re.search("[a-zA-Z]", text_input):
        return {"valid": False, "error": "Input must contain text."}

    return {"valid": True}


def log_prediction(
    text: str, prediction: Dict[str, Any], user_id: Optional[str] = None
) -> None:
    """
