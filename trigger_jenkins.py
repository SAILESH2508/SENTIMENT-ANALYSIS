...
def check_build_status(jenkins_url, job_name, username, token):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(f"❌ Failed to check build status. Status Code: {response.status_code}")
            return None, None, False
    except requests.exceptions.RequestException as e:
...
def main():
    ...
    parser.add_argument("--url", default="http://localhost:8080", help="Jenkins base server URL")
    parser.add_argument("--job", default="sentiment-analyzer", help="Jenkins job/pipeline name")
    parser.add_argument("--user", help="Jenkins username")
    parser.add_argument("--token", help="Jenkins API token or password")
    parser.add_argument("--action", choices=["trigger", "status", "logs", "monitor"], default="status",
                        help="Action to perform: trigger a build, get status, fetch logs, or trigger & monitor")
    parser.add_argument("--build", type=int, help="Build number (required for 'logs' if not querying the last build)")
    ...
    if args.action == "trigger":
        ...
        if trigger_job(args.url, args.job, args.user, args.token):
            ...
            # 2. Poll status
            while True:
                build_num, result, building = check_build_status(args.url, args.job, args.user, args.token)
                if not building and result is not None:
                    print(f"\n🏁 Build completed with outcome: {result}")
                    get_console_output(args.url, args.job, build_num, args.user, args.token)
                    break
                print("⏳ Job is still compiling in Jenkins pipeline. Checking again in 10 seconds...")
                time.sleep(10)
