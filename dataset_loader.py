...
class DatasetLoader:
        ...
        if os.path.exists(data_dir):
            for file in os.listdir(data_dir):
                if file.startswith('dataset_') and file.endswith('.csv'):
                    ...
                    name = file.replace('dataset_', '').replace('.csv', '').replace('_', ' ').title()
    ...
    def get_sample_texts(self, dataset_name: str, sentiment: str = None, count: int = 5) -> List[str]:
    ...
    def print_summary(self):
        ...
        for name, filename in self.available_datasets.items():
            try:
                ...
                sentiment_dist = df['sentiment'].value_counts() if 'sentiment' in df.columns else "N/A"
...
if __name__ == "__main__":
    ...
    if stats:
        print(f"  • Total Samples: {stats.get('generation_info', {}).get('total_samples', 'N/A'):,}")
        dist = stats.get('sentiment_distribution', {})
        if dist:
            print(f"  • Positive: {dist.get('positive', 0)} ({dist.get('positive_percentage', 0)}%)")
            print(f"  • Negative: {dist.get('negative', 0)} ({dist.get('negative_percentage', 0)}%)")
            print(f"  • Neutral: {dist.get('neutral', 0)} ({dist.get('neutral_percentage', 0)}%)")