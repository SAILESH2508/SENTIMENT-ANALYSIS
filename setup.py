...
def main():
    ...
    data_files_exist = (
        check_file_exists("data/train.csv", "Training data") and 
        check_file_exists("data/test.csv", "Test data")
    )
    ...
    if not check_file_exists("sentiment_pipeline.pkl", "Trained model"):
        print("\n🤖 Training model...")
        if not run_command("python model_trainer.py", "Training sentiment analysis model"):
            print("❌ Model training failed!")
            sys.exit(1)
    
    # Run tests
    print("\n🧪 Running tests...")
    if not run_command("python -m pytest test_sentiment_analyzer.py -v", "Running test suite"):
