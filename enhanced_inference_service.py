...
import hashlib
from utils import preprocess_text, validate_text_input, log_prediction, ensure_model_exists, ModelConfig
import logging
...
class SentimentAnalyzer:
    ...
    def predict_sentiment(self, text_input: str, use_cache: bool = True) -> Dict[str, Any]:
        """
        ...
        # Check cache first
        if use_cache and self.config.get('performance', {}).get('cache_predictions', True):
        ...
        try:
            ...
            lower_threshold = thresholds.get('intermediate_lower', ModelConfig.INTERMEDIATE_LOWER)
            upper_threshold = thresholds.get('intermediate_upper', ModelConfig.INTERMEDIATE_UPPER)
    ...
    def get_model_info(self) -> Dict[str, Any]:
        ...
        try:
            ...
            metrics_path = self.config.get('model', {}).get('metrics_path', 'model_metrics.pkl')
            metrics = {}
            try:
            ...
            return {
                ...
                'feature_count': len(self.pipeline.named_steps['tfidf'].get_feature_names_out()) if hasattr(self.pipeline.named_steps['tfidf'], 'get_feature_names_out') else 'Unknown',
